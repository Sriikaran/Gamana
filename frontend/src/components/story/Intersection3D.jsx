...
const TrafficSignal = ({ position, rotation, state }) => {
  return (
    <group position={position} rotation={rotation}>
      ...
      <mesh position={[0, 2.85, 0.71]}>
        <sphereGeometry args={[0.12, 16, 16]} />
        <meshStandardMaterial color="#33ff33" emissive="#00ff00" emissiveIntensity={state === 'green' ? 2.0 : 0} toneMapped={false} roughness={0.5} />
      </mesh>
    </group>
  );
};
...
export default function Intersection3D() {
  ...
  // Traffic Controller
  const [nsLight, setNsLight] = useState('green');
  const [ewLight, setEwLight] = useState('red');
  ...
  return (
    <>
      ...
      <TrafficSignal position={[-verticalRoadWidth / 2 - 0.6, 0.4, horizontalRoadWidth / 2 + 0.6]} rotation={[0, -Math.PI / 2, 0]} state={ewLight} />
      <TrafficSignal position={[verticalRoadWidth / 2 + 0.6, 0.4, -horizontalRoadWidth / 2 - 0.6]} rotation={[0, Math.PI / 2, 0]} state={ewLight} />

      {/* Vehicles */}
      <Vehicles3D nsLight={nsLight} ewLight={ewLight} />
    </>
  );
}
